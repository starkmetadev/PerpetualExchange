...
import "./interfaces/IGplxIou.sol";
import "./interfaces/IAmmRouter.sol";
import "./interfaces/IGplxMigrator.sol";

contract GplxMigrator is ReentrancyGuard, IGplxMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "GplxMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "GplxMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "GplxMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "GplxMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "GplxMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "GplxMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "GplxMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "GplxMigrator: invalid _lpTokenBs.length");

        isInitialized = true;

        ammRouter = _ammRouter;
        gplxPrice = _gplxPrice;
    ...
    function migrate(
        ...
        require(isMigrationActive, "GplxMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "GplxMigrator: token not whitelisted");
        require(_tokenAmount > 0, "GplxMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "GplxMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(gplxPrice);
        require(mintAmount > 0, "GplxMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "GplxMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "GplxMigrator: invalid tokenA");
            require(tokenB != address(0), "GplxMigrator: invalid tokenB");
        ...
        IGplxIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "GplxMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "GplxMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "GplxMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "GplxMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("GplxMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "GplxMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "GplxMigrator: invalid _action");
