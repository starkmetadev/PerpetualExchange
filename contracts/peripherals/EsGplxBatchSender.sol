...
contract EsGplxBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esGplx;

    constructor(address _esGplx) {
        admin = msg.sender;
        esGplx = _esGplx;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsGplxBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esGplx).transferFrom(msg.sender, _accounts[i], _amounts[i]);
