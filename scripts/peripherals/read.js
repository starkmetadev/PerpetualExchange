...
async function readGplpManager(gplpManager) {
  console.log("gplpManager.cooldownDuration", (await gplpManager.cooldownDuration()).toString())
}
...
async function main() {
  ...
  // await readGplpManager(gplpManager)

  // await getPool(weth.address, usdc.address, 500)
  // await getPool(weth.address, gplx.address, 10000)
  ...
  startTime = Date.now()
  await gplpManager.getAums()
  console.log("gplpManager.getAums", Date.now() - startTime)
}
