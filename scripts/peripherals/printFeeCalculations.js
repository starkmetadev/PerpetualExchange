...
async function getGplxPrice(ethPrice) {
...
async function getArbValues() {
  ...
  const stakedGplx = await contractAt("Token", "0xd2D1162512F927a7e282Ef43a362659E4F2a728F", signer)
  const stakedGplxSupply = await stakedGplx.totalSupply()
  const { totalTransferAmount: keeperCosts } = await getArbKeeperValues()
  const gplpManager = await contractAt("GplpManager", "0x321F653eED006AD1C29D174e17d96351BDe22649", signer)
  const gplpAum = await gplpManager.getAum(true)

  return { vault, reader, tokens, tokenInfo, nativeTokenPrice, feesUsd, stakedGplx, stakedGplxSupply, keeperCosts, gplpManager, gplpAum }
}
...
async function getAvaxValues() {
  ...
  const nativeTokenPrice = tokenInfo[tokens.nativeToken.address].maxPrice
  const feesUsd = await getFeesUsd(vault, reader, tokenInfo)
  const stakedGplx = await contractAt("Token", "0x4d268a7d4C16ceB5a606c173Bd974984343fea13", signer)
  const stakedGplxSupply = await stakedGplx.totalSupply()
  const { totalTransferAmount: keeperCosts } = await getAvaxKeeperValues()
  const gplpManager = await contractAt("GplpManager", "0xe1ae4d4b06A5Fe1fc288f6B4CD72f9F8323B107F", signer)
  const gplpAum = await gplpManager.getAum(true)
...
async function main() {
  ...
  const gplxPrice = await getGplxPrice(ethPrice)

  const data = [
    ["ETH Price", formatAmount(ethPrice, 30, 2)],
    ["AVAX Price", formatAmount(avaxPrice, 30, 2)],
    ["GPLX Price", formatAmount(gplxPrice, 30, 2)],
    ["ARB Fees", formatAmount(values.arbitrum.feesUsd, 30, 2)],
    ["AVAX Fees", formatAmount(values.avax.feesUsd, 30, 2)],
    ["ARB sbfGPLX", formatAmount(values.arbitrum.stakedGplxSupply, 18, 2)],
    ["AVAX sbfGPLX", formatAmount(values.avax.stakedGplxSupply, 18, 2)],
    ...
    ["GPLP AUM (ARB)", formatAmount(values.arbitrum.gplpAum, 30, 2)],
    ["GPLP AUM (AVAX)", formatAmount(values.avax.gplpAum, 30, 2)],
  ]
