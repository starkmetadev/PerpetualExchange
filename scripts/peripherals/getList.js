...
const holderList = require("../../data/holders/gplxHolders.json")
...
async function main() {
  ...
  for (let i = 3490; i < holderList.length; i++) {
    ...
    const bnGplxBalance = await feeGplxTracker.depositBalances(account, bnGplx.address)
    const pendingRewards = await bonusGplxTracker.claimable(account)
    const totalRewards = bnGplxBalance.add(pendingRewards)
    console.log(`${i+1},${account},${ethers.utils.formatUnits(bnGplxBalance, 18)},${ethers.utils.formatUnits(pendingRewards, 18)},${ethers.utils.formatUnits(totalRewards)}`)
