...
async function getArbValues() {
  ...
  const esGplx = await contractAt("Token", "0xf42Ae1D54fd613C9bb14810b0588FaAa09a426cA")
  const nativeTokenPrice = ethPrice
  const data = arbitrumData

  return { batchSender, esGplx, nativeTokenPrice, data }
}

async function getAvaxValues() {
  const batchSender = await contractAt("BatchSender", "0xF0f929162751DD723fBa5b86A9B3C88Dc1D4957b")
  const esGplx = await contractAt("Token", "0xFf1489227BbAAC61a9209A08929E4c2a526DdD17")
...
async function main() {
  ...
  const { batchSender, esGplx, nativeTokenPrice, data } = await getValues()
  ...
  let totalEsGplxAmount = bigNumberify(0)
  const affiliateAccounts = []
  const affiliateAmounts = []
  const discountAccounts = []
  const discountAmounts = []
  const esGplxAccounts = []
  const esGplxAmounts = []

  for (let i = 0; i < affiliatesData.length; i++) {
    const { account, rebateUsd, esgplxRewardsUsd } = affiliatesData[i]
    ...
    if (esgplxRewardsUsd) {
      const esGplxAmount = bigNumberify(esgplxRewardsUsd).mul(expandDecimals(1, 18)).div(expandDecimals(gplxPrice, 30))
      esGplxAccounts.push(account)
      esGplxAmounts.push(esGplxAmount)
      totalEsGplxAmount = totalEsGplxAmount.add(esGplxAmount)
    }
  }
  ...
  console.log(`total esGplx`, ethers.utils.formatUnits(totalEsGplxAmount, 18))
  ...
  if (shouldSendTxn) {
    ...
    await sendTxn(esGplx.approve(batchSender.address, totalEsGplxAmount), "esGplx.approve")

    await processBatch([esGplxAccounts, esGplxAmounts], batchSize, async (currentBatch) => {
      printBatch(currentBatch)

      const accounts = currentBatch.map((item) => item[0])
      const amounts = currentBatch.map((item) => item[1])

      await sendTxn(batchSender.sendAndEmit(esGplx.address, accounts, amounts, affiliateRewardsTypeId), "batchSender.sendAndEmit(nativeToken, esGplx affiliate rewards)")
    })
  }
}
