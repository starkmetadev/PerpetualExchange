...
const ARBITRUM_SUBGRAPH_ENDPOINT = 'https://api.thegraph.com/subgraphs/name/gplx-io/gplx-arbitrum-referrals'
const AVALANCHE_SUBGRAPH_ENDPOINT = 'https://api.thegraph.com/subgraphs/name/gplx-io/gplx-avalanche-referrals'
const CRONOS_SUBGRAPH_ENDPOINT = 'https://graph-node.gplxespade.xyz/subgraphs/name/graphprotocol/referrals'

const BigNumber = ethers.BigNumber
const { formatUnits, parseUnits } = ethers.utils
const SHARE_DIVISOR = BigNumber.from("1000000000") // 1e9
const BONUS_TIER = 2 // for EsGPLX distributions
const USD_DECIMALS = 30
const GPLX_DECIMALS = 18
...
async function queryDistributionData(network, fromTimestamp, toTimestamp, account, gplxPrice, esgplxRewards) {
  if (gplxPrice) {
    gplxPrice = parseUnits(gplxPrice, USD_DECIMALS)
  }
  if (esgplxRewards) {
    esgplxRewards = parseUnits(esgplxRewards, GPLX_DECIMALS)
  }
  ...
  if (gplxPrice && esgplxRewards) {
    const esgplxRewardsUsdLimit = esgplxRewards.mul(gplxPrice).div(expandDecimals(1, GPLX_DECIMALS))
    let esgplxRewardsUsdTotal = BigNumber.from(0)
    Object.values(referrersRebatesData).forEach(data => {
      if (data.tierId !== BONUS_TIER) {
        return
      }
      data.esgplxRewardsUsd = data.volume.div(1000).div(20) // 0.1% margin fee, 0.05% of fee is EsGPLX bonus rewards
      data.esgplxRewards = data.esgplxRewardsUsd
        .mul(expandDecimals(1, USD_DECIMALS))
        .div(gplxPrice)
        .div(expandDecimals(1, 12))
      esgplxRewardsUsdTotal = esgplxRewardsUsdTotal.add(data.esgplxRewardsUsd)
    })

    if (esgplxRewardsUsdTotal.gt(esgplxRewardsUsdLimit)) {
      const denominator = esgplxRewardsUsdTotal.mul(USD_DECIMALS).div(esgplxRewardsUsdLimit)
      Object.values(referrersRebatesData).forEach(data => {
        data.esgplxRewardsUsd = data.esgplxRewardsUsd.mul(USD_DECIMALS).div(denominator)
        data.esgplxRewards = data.esgplxRewardsUsd
          .mul(expandDecimals(1, USD_DECIMALS))
          .div(gplxPrice)
          .div(expandDecimals(1, 12))
  ...
  for (const data of Object.values(referrersRebatesData)) {
    ...
    consoleData.push({
      ...
      tierId: data.tierId,
      "esgplxRewards, $": data.esgplxRewardsUsd ? formatUnits(data.esgplxRewardsUsd, USD_DECIMALS) : null,
      esgplxRewards: data.esgplxRewards ? formatUnits(data.esgplxRewards, GPLX_DECIMALS) : null,
    })
    output.referrers.push({
      ...
      tierId: data.tierId,
      esgplxRewards: data.esgplxRewards ? data.esgplxRewards.toString() : null,
      esgplxRewardsUsd: data.esgplxRewardsUsd ? data.esgplxRewardsUsd.toString() : null,
    })
  }
...
async function main() {
  ...
  parser.add_argument('-g', '--gplx-price', { help: 'GPLX TWAP price' })
  parser.add_argument('-e', '--5000', {
    help: 'Amount of EsGPLX to distribute to Tier 3',
    default: "5000"
  })
