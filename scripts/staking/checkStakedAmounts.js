...
const stakeGplxList = require("../../data/stakeGplxList.json")
...
async function main() {
  ...
  for (let i = 0; i < stakeGplxList.length; i++) {
    const { address, balance } = stakeGplxList[i]

    const stakedAmount = await stakedGplxTracker.stakedAmounts(address)
    ...
    const pendingRewards = await stakedGplxTracker.claimable(address)
    const pendingBonus = await bonusGplxTracker.claimable(address)
