...
const distribute = async () => {
  ...
  const stakedGplxBalances = await processFile(inputDir + "staked-gplx-balances.csv")
  const vestedBalances = await processFile(inputDir + "vested-balances.csv")

  const holders = {}

  for (let i = 0; i < stakedGplxBalances.length; i++) {
    const tokenHolder = stakedGplxBalances[i]
  ...
  if (shouldSendTokens) {
    await sendTxn(esGplx.approve(batchSender.address, expandDecimals(totalEsGplx, 18)), "esGplx.approve")
  }
  ...
  for (let i = 0; i < balanceList.length; i++) {
    ...
    const esGplxValue = (totalEsGplx - 1) * balance / totalBalance
    const esGplxAmount = ethers.utils.parseUnits(esGplxValue.toFixed(4), 18)

    accounts.push(account)
    amounts.push(esGplxAmount)
    totalEsGplxAmount = totalEsGplxAmount.add(esGplxAmount)

    console.log(`${i+1},${account},${esGplxValue},${esGplxAmount.toString()}`)

    if (accounts.length === batchSize && shouldSendTokens) {
      console.log("sending batch", i, accounts.length, amounts.length)
      await sendTxn(batchSender.send(esGplx.address,  accounts, amounts), "batchSender.send")
  ...
  console.log("totalEsGplxAmount", totalEsGplxAmount.toString())
}
